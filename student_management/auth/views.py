...
register_admin_serializer = auth_namespace.model('Register serializer', register_admin_fields_serializer)
...
class StudentRegistrationView(Resource):
    ...
    def post(self):
        ...
        if  data.get('user_type') == "student":
            ...
            employee_number = random_char(10)
            new_user = Teacher(
            email=data.get('email'), 
            identifier=identifier,
            first_name=data.get('first_name'),
            ...
            return {"message":" Invalid user type "}, HTTPStatus.BAD_REQUEST
...
class AdminCreationView(Resource):
    ...
    def post(self):
        ...
        user = Admin.query.filter_by(school_mail=data.get('school_email', None)).first()
        if user:
            return {'message': 'School email already exist'} , HTTPStatus.CONFLICT
        # Create new user
        identifier=random_char(10)  
        designation= 'Principal'
        new_user = Admin(
